class Reimbursement {
    constructor(amount, description, author, username, resolver, submitted, resolved, type, status) {
        this.amount = amount;
...
async function submitTicket(reimbursement) {
    try {
...
function clearForm() {
    document.getElementById('money').value = ''; //Weirdly it REQUIRED the ;
...
function makeInfoCard(reimbursement) {
    ...
    if (resolved)
        resolved.appendChild(resolver);
    if (resolved)
        resolved.appendChild(resDate);
